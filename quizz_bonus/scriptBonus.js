  ...
  this.displayCurrentQuestion = function () {
    if (indexCurrentQuestion < this.questions.length) {
      ...
      if (quiz.nbrCorrects >= 6) {
        // si réponses >= 6, ajout d'image trophy + texte congrats
      ...
      // ***** NGE -- prompt pour entrer nom du joueur ***** //
      result_screen.style.display = "block";
      ...
      nameForm.addEventListener("submit", function (event) {
        ...
        // Pour desactive le formulaire apres validation
...
let question1 = new Question(
  "Dans quelle animé, peut-on rencontrer la pieuvre Hank ? ",
  ["La Petite Sirène", "Atlantide", "Le Monde de Dory"],
  [3]
);
quiz.addQuestion(question1);

let question2 = new Question(
  "Comment s'appelle le poulpe collègue de Bob l'éponge ? ",
  ["Carlo", "Patrick", "Michel"],
  [1]
);
quiz.addQuestion(question2);

let question3 = new Question(
  "Qui est Ursula dans l'univers Walt Disney ? ",
  [
    "Un personnage de Lilo et Stitch ",
    "La méchante dans la Petite sirène",
    "Personne c'est le nom de ma tante",
  ],
  [2]
);
...
let question4 = new Question(
  "Dans quel film peut-on apercevoir Blip-Bloop la pieuvre ? ",
  ["Peter Pan 2", "Peter Pan 1", "Vaïana"],
  [1]
);
quiz.addQuestion(question4);

let question5 = new Question(
  "Dans quel roman d'aventure paru en 1870, le héro se confronte au Kraken ? ",
  [
...
let question6 = new Question(
  "Comment s'appelle le pirate à tête de poulpe ennemie de Jack Sparrow ? ",
  ["Armando Salazar", "Barbe Noire", "Davy Jones"],
  [3]
);
quiz.addQuestion(question6);

let question7 = new Question(
  "Qui Spiderman affront-il dans le second volet sorti en 2004 ? ",
  ["Dc Octopus", "Le bouffon Vert", "Venom"],
  [1]
);
quiz.addQuestion(question7);

let question8 = new Question(
  "Dans quel manga/animé peut-on découvrir Hatchan (Octo en français) ? ",
  ["One Piece", "Dragon ball", "Demon Slayer"],
  [1]
);
quiz.addQuestion(question8);

let question9 = new Question(
  "Qui est Célia Mae dans Monstre & Cie ? ",
  ["La chef des monstres", "La petite fille", "La copine de Bob Razowski"],
  [3]
);
quiz.addQuestion(question9);

let question10 = new Question(
  "Dans quel film, des pieuvres mécaniques patrouillent sous la surface de la planète ? ",
...
function saveHighScores() {
  localStorage.setItem("highScoresQ2", JSON.stringify(highScores));
}
// charge le score
function loadHighScores() {
  const savedHighScores = localStorage.getItem("highScoresQ2");
  if (savedHighScores) {
...
function displayHighScores() {
  const highScoresList = document.getElementById("highScoresQ2List");
  ...
  // Afficher les HighScore

  const highScoresListContainer = document.getElementById(
    "highScoresQ2ListContainer"
  );
  highScoresListContainer.style.display = "block";

  // Ajout bouton pour fermer et cacher les highscores
...
// pour vider le local storage dans la console du DOM utiliser la methode localStorage.removeItem("highScoresQ2");
